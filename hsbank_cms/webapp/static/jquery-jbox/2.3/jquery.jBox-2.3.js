...
(function (a) {
	a.jBox = function (b, c) {
		...
		j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:' + (0x6 + c.border) + 'px; right:' + (0x6 + c.border) + 'px; width:16px; height:16px;' + (c.showClose ? '' : 'display:none;') + '"></a>');
		if (g) {
		...
		var u = a.browser.msie ? 'line-height:19px;padding:0px 6px 0px 6px;' : 'padding:0px 26px 0px 26px;';
		a.each(b, function (N, O) {
			...
			r.push('<div class="jbox-button-panel" style="height:30px;padding:20px 0 20px 0;text-align: center;' + (P ? '' : 'display:none;') + '">');
			if (!c.isTip) {
				r.push('<span class="jbox-bottom-text" style="float:left;display:block;line-height:25px;"></span>')
			};
			a.each(O.buttons, function (T, U) {
				if(U == 1) {
					r.push('<button class="jbox-button jbox-button-brown" value="' + U + '" style="' + u + '">' + T + '</button>')
				} else if(U == 0) {
					r.push('<button class="jbox-button jbox-button-blue" value="' + U + '" style="' + u + '">' + T + '</button>')
				} else if(U == -1) {
					r.push('<button class="jbox-button jbox-button-gray" value="' + U + '" style="' + u + '">' + T + '</button>')
				} else {
	...
	a.jBox.prompt = function (b, c, d, e) {
		...
		var g = e.icon == 'none' ? 'height:auto;padding:30px 10px;' : 'padding:30px 0;' + ((a.browser.msie && parseInt(a.browser.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var h = [];
		h.push('html:');
		h.push('<div style="margin:10px;' + g + 'padding-left:' + (e.icon == 'none' ? 0x9 : 0x32) + 'px;text-align:left;">');
		h.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:' + (f + e.border + 55) + 'px;left:' + (0xa + e.border + 10) + 'px; width:32px; height:32px;"></span>');
		h.push(e.content);
		h.push('</div>');
		e.content = h.join('');
		a.jBox(e.content, e)
	...
	a.jBox.tip = function (b, c, d) {
		...
		var e = [];
		e.push('html:');
		e.push('<div style="min-height:68px; height:auto; line-height:68px; margin:22px 30px;padding-left:50px;padding-top:0px;text-align:left;">');
		e.push('<span class="jbox-icon jbox-icon-' + d.icon + '" style="position:absolute;top:' + (0x26 + d.border) + 'px;left:' + (0x20 + d.border) + 'px; width:32px; height:32px;"></span>');
		e.push(d.content);
		e.push('</div>');
		d.content = e.join('');
	...
	a.jBox.messager = function (b, c, d, e) {
		...
		i.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:' + (g + e.border + 20) + 'px;left:' + (0xa + e.border) + 'px; width:32px; height:32px;"></span>');
		i.push(e.content);
		i.push('</div>');
		e.content = i.join('');
		a.jBox(e.content, e)

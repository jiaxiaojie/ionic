...
$.extend($.validator, {
	...
	prototype: {
		...
		defaultShowErrors: function() {
			...
			this.addWrapper( this.toShow ).show().fadeOut(3500);
		},
	...
	methods: {
		...
		// http://docs.jquery.com/Plugins/Validation/Methods/min_more
		minmore: function( value, element, param ) {
			return this.optional(element) || (!isNaN(value) && !isNaN(param) && Number(value) > Number(param));
		},
		...
		range: function( value, element, param ) {
			return this.optional(element) || (!isNaN(value) && !isNaN(param[0]) && !isNaN(param[1]) && Number(value) >= Number(param[0]) && Number(value) <= Number(param[1]));
		},
...
(function ($) {
	$.extend($.validator.messages, {
		...
		minmore: $.validator.format("请输入一个大于 {0} 的值")
	});
}(jQuery));
...
},"3-20位字母或数字开头，允许字母数字下划线，首位必须为字母");
...
jQuery.validator.addMethod("amount", function(value, element) {
    return this.optional(element) || (/^(([1-9]\d*)|0)(\.\d{1,2})?$/.test(value) && (value != 0));
}, "请输入正确的金额，小数部分最多2位");
 ...
 * @param inputStr
 */
function isMobile(fieldValue) {
	return isCmccMobile(fieldValue) || isCutcMobile(fieldValue) || isCtccMobile(fieldValue);
}
 ...
 * @param inputStr
 */
function isCmccMobile(inputStr) {
 ...
 * @param inputStr
 */
function isCutcMobile(inputStr) {
	var reg = /(^13[0-2]\d{8}$)|(^14[1,3,5]\d{8}$)|(^15[5,6]\d{8}$)|(^170[7,8,9]\d{7}$)|(^17[5,6]\d{8}$)|(^18[5,6]\d{8}$)/;
	return reg.test(inputStr);
}
 ...
 * @param inputStr
 */
function isCtccMobile(inputStr) {		

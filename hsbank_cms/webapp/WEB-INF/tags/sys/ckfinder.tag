...
<%@ attribute name="canOpenImg" type="java.lang.Boolean" required="false" description="点击图片查看原图"%>
...
<%@ attribute name="maxSelect" type="java.lang.Integer" required="false" description="文件最大上传数"%>
<%@ attribute name="first" type="java.lang.Boolean" required="false" description="是否为第一个选择框控件"%>
<ol id="${input}Preview"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn">清除</a>
<c:if test="${selectMultiple and maxSelect != null and maxSelect>0}">
&nbsp;您最多只能选择${maxSelect}个文件
</c:if>
...
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		var maxSelect = ${maxSelect==null?"null":maxSelect};
		var selectMultiple = ${selectMultiple==null?true:selectMultiple};
		if(maxSelect != null && maxSelect>0 && selectMultiple){
			var urlLength = 0;
			var urls = $("#${input}").val().split("|");
			for(var j = 0; j < urls.length;j++){
				if(urls[j] != null && urls[j] != ""){
					urlLength++;
				}
			}
			
			var count = urlLength+files.length;
			if(count > maxSelect){
				var duo = count-maxSelect;
				mylength = mylength- duo;
			}
		}
		
		
		for(var i=0; i<mylength; i++){//<c:if test="${type eq 'thumb'}">
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li>";//<c:if test="${canOpenImg!=false}">
				li += "<a data-lightbox='correlation-image-${input}' data-title='相关文件' href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\">";//</c:if>
				li += "<img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//<c:if test="${canOpenImg != false}">
				li += "</a>";//</c:if></c:if><c:if test="${type ne 'thumb' && type ne 'images'}">

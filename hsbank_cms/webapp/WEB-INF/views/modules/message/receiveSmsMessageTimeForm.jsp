...
<head>
	...
	function initSelect(){
		...
		$(function(){
			...
			$("#systemMessageTimeBtn").click(function(){
				refreshFormByAccountId(0);
			
			});
			if($("#accountId").val()=='0'){
				$("#textId_accountId").val("系统默认设置");
			}
			
		});
		
		function refreshForm(){
			refreshFormByAccountId($("#accountId").val())
		}
		function refreshFormByAccountId(accountId){
			var a = $("#menu_2").find(".active").find("a");
			
			post(a.attr("href"), "disabledForm=false&accountId="+accountId);
		}
	...
	<form:form id="inputForm" modelAttribute="receiveSmsMessageTime" action="${ctx}/message/receiveSmsMessageTime/save?disabledForm=false" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<input id="btnSubmit" class="btn btn-primary" type="submit" onclick="refreshForm();" value="刷新"/>&nbsp;&nbsp;<input id="systemMessageTimeBtn" class="btn btn-primary" type="button"  value="查询系统默认设置"/>
			</div>
		</div>
		<div class="control-group">

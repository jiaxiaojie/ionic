...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/project/projectBaseInfo/investmentlist?accountId=${projectBaseInfo.customerAccountId}">相关投资项目</a></li>
		<li class="active"><a href="${ctx}/project/projectBaseInfo/transferlist?accountId=${projectBaseInfo.customerAccountId}">相关转让项目</a></li>
	</ul><br/>
		<form:form id="searchForm" modelAttribute="projectBaseInfo" action="${ctx}/project/projectBaseInfo/transferlist" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="projectTransferInfo">
			<tr>
				...
				<td>
				<fmt:formatNumber value="${pti.tansferPrice}" pattern="#0.00"/>
				</td>
				<td>
					<fmt:formatDate value="${pti.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					${fns:getDictLabel(pti.status, 'project_transfer_status_dict', '')}
				</td>
			</tr>
		</c:forEach>
		</tbody>

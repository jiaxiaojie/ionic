		...
		<form:form id="authorizeAutoRepaymentForm" modelAttribute="currentProjectInfo" action="${authorizeAutoRepaymentUrl }" method="post" class="form-horizontal" target="_blank">
		    <c:choose>
		    	<c:when test="${isSuccess }">
		    	  <div class="control-group">活期产品投资成功！</div>
		    	   <div class="control-group">
		        	<p>您已成功完成投资，<span id="remainSeconds" style="color:red;"></span>秒钟后会跳转至产品列表，如果没有跳转，请直接点击</p>
		        </div>
		        </c:when>
		        <c:otherwise>
		           <div class="control-group">投资失败，请重试</div>
		        </c:otherwise>
		    </c:choose>

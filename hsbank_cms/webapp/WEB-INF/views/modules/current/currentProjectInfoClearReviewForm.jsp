...
<head>
	...
	function formatSelection(repo) {
		...
		$(document).ready(function() {
			...
			var selects = document.getElementsByName("windingUpStatus"); 
		    selects[0].checked= true; 
			$(":input").not($("#btnSubmit")).not($("#windingUpReviewRemark")).not($("[name='windingUpStatus']")).not($("#btnCancel")).not($("[type=hidden]")).attr("disabled",true);
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/current/currentProjectInfo/clearReviewList">活期项目清盘审批列表</a></li>
		<li class="active"><a href="${ctx}/current/currentProjectInfo/clearReviewForm?id=${currentProjectInfo.id}">活期项目信息清盘审批</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="currentProjectInfo" action="${ctx}/current/currentProjectInfo/clearReview" method="post" class="form-horizontal">
		<form:hidden path="id"/>
...
${currentProjectInfo.interestCalculateRule}</textarea>
		...
		<div class="control-group">
			<label class="control-label">清盘审核状态：</label>
			<div class="controls">
				<form:radiobutton path="windingUpStatus"  value="2"  class="required"/> 通过
				<form:radiobutton path="windingUpStatus"  value="0"  class="required"/>不通过
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">清盘审核意见：</label>
			<div class="controls">
				<form:textarea path="windingUpReviewRemark" htmlEscape="false" maxlength="500" rows="4"  class="input-xlarge required"/>
			</div>
		</div>
		
		<div class="form-actions">
			<shiro:hasPermission name="current:currentProjectInfo:clearReview">
				<input id="btnSubmit" class="btn btn-primary" type="submit" value="审批"/>
			</shiro:hasPermission>

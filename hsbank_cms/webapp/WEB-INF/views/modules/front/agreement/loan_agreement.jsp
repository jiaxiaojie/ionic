...
<div class="div_bg_001">
	<div class="agreement_area">
    	<div class="agreement">
            <div class="title"><span>借款协议书</span><p>（个人消费类）</p></div>
            ...
            <div class="loan_field">
            	<dl class="clearfix">
                	<dt>乙方（借款人）：</dt>
                    <dd>${p2p:vagueName(customerBase.customerName) }</dd>
                </dl>
            	<dl class="clearfix">
                	<dt>花生金服用户名：</dt>
            	...
            	<dl class="clearfix">
                	<dt>通讯地址：</dt>
                    <dd>${p2p:vagueAddress(customerBase.address) }</dd>
                </dl>
            	<dl class="clearfix">
                	<dt>联系电话：</dt>
                    <dd>${p2p:vagueMobile(customerBase.mobile) }</dd>
                </dl>
            </div>
            <div class="loan_field">
            	<dl class="clearfix">
            ...
            <dl class="item">
                <dt>鉴于：</dt>
                <dd>1、甲方承诺对本协议涉及的借款具有完全的支配能力，是其自有闲散资金，为其合法所得；并承诺其提供给丙方的信息是完全真实、有效、合法的。</dd>
                <dd>2、乙方有借款需求，甲方亦同意借款，双方建立借贷关系均系真实意思表示。</dd>
                <dd>3、乙方已在花生金服网站注册，并承诺其提供给丙方的信息是完全真实有效的。</dd>
                <dd>4、丙方是一家在上海市合法成立并有效存续的股份有限责任公司，拥有 <a href="http://www.hsbank360.com" target="_blank" class="btn_text_gold">www.hsbank360.com</a>  网站（下称“花生金服”）的经营权，提供信息咨询，为交易方提供信息中介服务。</dd>
                	...
                	<table class="table_loan" cellpadding="0" cellspacing="0" border="1">
                    	<tbody>
                        	<tr>
                                ...
                                <td width="83%"><p><fmt:formatNumber value="${loanMoney }" pattern="#0.00"/>元</p></td>
                            </tr>
                        	<tr>
                        		<td height="40" align="center">借款期限</td>
                                <td><p>自<span>${startYear }</span>年<span>${startMonth }</span>月<span>${startDay }</span>日始至<span>${endYear }</span>年<span>${endMonth }</span>月<span>${endDay }</span>日止。</p></td>
                            </tr>
                        	<tr>
                        	...
                        	<tr>
                                ...
                                <td><p>${fns:getDictLabel(useType, 'customer_credit_loan_use_dict', '')}</p></td>
                            </tr>
                        	<tr>
                        		<td height="62" align="center">还款日</td>
                                <td><p>项目满标后，从投标截止日起，每月对应日 （若当月没有该日，则还款日为当月最后一天，节假日不顺延）。到期还本付息的，还款日为借款到期日。</p></td>
                            </tr>
                        </tbody>
                    </table>
                </dd>
            ...
            <dl class="item">
                <dt>第四条、乙方愿意按照以下第    种方式偿还借款本息，直至乙方还清全部借款本息为止：</dt>
                ...
                <dd>（三）、采用分期付息，到期一次还本的，每期利息计算公式为： 每期利息 = 期初贷款剩余本金×期利率。</dd>
                <dd>（四）、采用到期一次性归还本息、利随本清的，计息方式为：贷款利息采取对年、对月、对日的计算方法，如果不能对年、对月、对日，以月末最后一日为对日。贷款期限满年的按年利率计算，满月的按月利率计算，有整年（月）又有零头天数的，整年（月）部分按 年（月）利率计算，仅零头天数按实际天数计算，其计算公式为： 全是整年整月的：利息=本金×时期（年或月数）×年或月利率； 有整年（月）又有零头天数的：利息=本金×[时期（年或月数）×年或月利率+零头天数×日利率]</dd>
                <dd>注：月利率=年利率/12，日利率=年利率/360，期利率=年利率×每期月数/12。</dd>
            ...
            <dl class="item">
                ...
                <dd>（三）、乙方申请全部提前偿还借款本金的，甲方按照提前还本时本协议执行的借款利率、提前还本的金额以及当期的实际用款天数计收利息，此前已计收的借款利息不再调整，同时乙方应当足额支付剩余借款本金2%的违约金，其中甲方、丙方各收取1%。</dd>
            </dl>
            <dl class="item">
            ...
            <dl class="item">
                ...
                <dd>（二）、乙方知晓并同意，甲方持有债权30日后于本协议履行过程中可能将其享有的债权的全部或部分转让给不特定的第三人，且转让次数无限定。甲方根据本协议转让借款债权的，无须事先通知乙方，债权受让人依法承接甲方在本协议项下享有的权利和义务。</dd>
            </dl>
            <dl class="item">
                <dt>第十条、委托代理确认</dt>
            ...
            <dl class="item">
                <dt>第十一条、借款担保</dt>
                <dd>（一）、担保方式为<span class="underline"></span>。</dd>
                <dd>1、保证担保范围</dd>
                <dd>借款金额（大写）<span class="underline">${upperMoney }</span>元及利息、借款人应支付的违约金（包括罚息）、赔偿金以及实现借款债权和抵押权的费用（包括但不限于律师费、保全费、诉讼费、合理费用等）。</dd>
                <dd>2、保证方式</dd>
            ...
            <dl class="item">
                ...
                <dd>1、本协议生效后，无论何种原因导致乙方未能发生本协议项下的消费行为；</dd>
            ...
            <div class="loan_field">
            	...
            	<dl class="clearfix">
                	<dt>日期：</dt>
                    <dd>${startYear }年${startMonth }月${startDay }日</dd>
                </dl>
            </div>
            <div class="loan_field">
            	<dl class="clearfix">

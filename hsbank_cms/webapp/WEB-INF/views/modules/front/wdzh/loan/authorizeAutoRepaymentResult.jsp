...
<html>
	<head>
		...
		<title>授权自动还款<c:choose><c:when test="${isSuccess }">成功</c:when><c:otherwise>失败</c:otherwise></c:choose></title>
		<c:if test="${isSuccess }">
	...
	<body>
		...
		<div class="content980">
			<div class="zjgl_xxqr">
		    	...
		    	<div class="xxqr_title blue-text"><i class="blue_check"></i>自动还款授权成功！</div>
		        <div class="zhgl_wxkt">
		        	<p>您已成功完成授权自动还款，<span id="remainSeconds" style="color:red;"></span>秒钟后会跳转至我的融资，如果没有跳转，请直接点击</p>
		        </div>
		    	</c:when>
		    	<c:otherwise>
		    	<div class="xxqr_title red-text"><i class="red_check"></i>自动还款授权失败！</div>
		        <div class="zhgl_wxkt">
		        	<p>授权自动还款失败，请重试</p>
		        </div>
		    	</c:otherwise>
		    	</c:choose>
		        <div class="btn_text"><a href="${ctxFront }/customer/loan/myFundingProject">立即跳转</a></div>
		    </div>

	...
	<body>		
		...
		<div id="content_center" class="bg_789_middle" style="width:769px;">
			<div id="tab_bar" style="padding-left:20px;">
				<li id="item_1" onClick="selectItem(1)" class="item"><span class="selected">月排行</span></li>
				<li id="item_2" onClick="selectItem(2)" class="item"><span>周排行</span></li>
			</div>
			<!-- 待收款明细 -->
			<div id="record_list" class="show">
				<div class="div_height_15"></div>
				<div class="line_01">
					<form id="searchForm" method="post" action="${ctxFront }/customer/investmentRank/rankMonth" style="overflow:hidden;">
						<input type="hidden" name="pageNo" id="pageNo" value="${page.pageNo }"/>
						<span class="span_text_1">年</span>
						<form:select id="year" name="year" path="rank.year" class="select">
		                    <form:options items="${fns:getDictList('investment_rank_year_dict')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		                </form:select>
		                <span class="span_text_1">月</span>
						<form:select id="month" name="month" path="rank.month" class="select">
		                    <form:options items="${fns:getDictList('investment_rank_month_dict')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		                </form:select>

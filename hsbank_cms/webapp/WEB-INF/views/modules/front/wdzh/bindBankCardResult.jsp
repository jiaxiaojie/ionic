...
<html>
	<head>
		...
		<title>绑定银行卡<c:choose><c:when test="${isSuccess }">成功</c:when><c:otherwise>失败</c:otherwise></c:choose></title>
		<c:if test="${isSuccess }">
	...
	<body>
		...
		<div class="content980">
			<div class="zjgl_xxqr">
		    	...
		    	<div class="xxqr_title blue-text"><i class="blue_check"></i>绑定银行卡成功！</div>
		        <div class="zhgl_wxkt">
		        	<p>您已成功绑定银行卡，<span id="remainSeconds" style="color:red;"></span>秒钟后会跳转至我的账户，如果没有跳转，请直接点击</p>
		        </div>
		    	</c:when>
		    	<c:otherwise>
		    	<div class="xxqr_title red-text"><i class="red_check"></i>绑定银行卡失败！</div>
		        <div class="zhgl_wxkt">
		        	<p>绑定银行卡失败，请重试</p>
		        </div>
		    	</c:otherwise>

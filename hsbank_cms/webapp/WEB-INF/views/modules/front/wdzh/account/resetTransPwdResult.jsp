...
<html>
	<head>
		...
		<title>重置交易密码<c:choose><c:when test="${isSuccess }">成功</c:when><c:otherwise>失败</c:otherwise></c:choose></title>
		<c:if test="${isSuccess }">
	...
	<body>
		<div class="content980">
			<div class="zjgl_xxqr">
		    	...
		    	<div class="xxqr_title blue-text"><i class="blue_check"></i>交易密码重置成功！</div>
		        <div class="zhgl_wxkt">
		        	<p>页面会在<span id="remainSeconds" style="color:red;"></span>秒钟后跳转至安全中心。如果没有跳转，请直接点击</p>
		        </div>
		    	</c:when>
		    	<c:otherwise>
		    	<div class="xxqr_title red-text"><i class="red_check"></i>交易密码重置失败！</div>
		        <div class="zhgl_wxkt">
		        	<p>交易密码重置失败，请重试</p>
		        </div>
		    	</c:otherwise>

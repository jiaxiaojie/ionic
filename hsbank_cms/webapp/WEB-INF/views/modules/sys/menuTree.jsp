		    ...
		    <div id="collapse-${menu.id}" class="accordion-body collapse ${not empty firstMenu && firstMenu ? 'in' : ''}">
				<div class="accordion-inner">
					...
					<c:if test="${menu2.parent.id eq menu.id&&menu2.isShow eq '1'}">
						<c:if test="${fn:indexOf(menu2.href, 'dataQuery/dataQuery') ne -1 }">
							<c:set var="joinMenuId" value="?menuId=${menu2.id }"></c:set>
						</c:if>
						<li><a data-href=".menu3-${menu2.id}" href="${fn:indexOf(menu2.href, '://') eq -1 ? ctx : ''}${not empty menu2.href ? menu2.href : '/404'}${joinMenuId}" target="${not empty menu2.target ? menu2.target : 'mainFrame'}" ><i style="color: #333;font-size: 14px" class="${not empty menu2.icon ? menu2.icon : 'circle-arrow-right'}"></i>&nbsp;${menu2.name}</a>
							...
							<c:forEach items="${menuList}" var="menu3">
								...
								<li class="menu3-${menu2.id} hide"><a href="${fn:indexOf(menu3.href, '://') eq -1 ? ctx : ''}${not empty menu3.href ? menu3.href : '/404'}" target="${not empty menu3.target ? menu3.target : 'mainFrame'}" ><i style="color: #333;font-size: 14px" class="${not empty menu3.icon ? menu3.icon : 'circle-arrow-right'}"></i>&nbsp;${menu3.name}</a></li>
								</c:if>
							</c:forEach>
							</ul></li><c:set var="firstMenu" value="false"/>
					</c:if>
					</c:forEach></ul>
				</div>
		    </div>

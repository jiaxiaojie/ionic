...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/customer/customerWithdrawHis/">会员体现记录表列表</a></li>
		<shiro:hasPermission name="customer:customerWithdrawHis:edit"><li><a href="${ctx}/customer/customerWithdrawHis/form">会员体现记录表添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="customerWithdrawHis" action="${ctx}/customer/customerWithdrawHis/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>体现银行卡号：</label>
			...
			<li><label>操作时间：</label>
				<input name="beginOpDt" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${customerWithdrawHis.beginOpDt}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
				<input name="endOpDt" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${customerWithdrawHis.endOpDt}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="customerWithdrawHis">
			<tr>
				<td><a href="${ctx}/customer/customerWithdrawHis/form?id=${customerWithdrawHis.id}">
					${customerWithdrawHis.accountId}
				</a></td>
				<td>
					${customerWithdrawHis.amount}
				</td>
				<td>
					${customerWithdrawHis.bankCardNo}
				</td>
				<td>
					${customerWithdrawHis.bankCode}
				</td>
				<td>

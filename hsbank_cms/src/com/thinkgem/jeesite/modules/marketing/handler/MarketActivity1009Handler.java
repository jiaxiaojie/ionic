 ...
 *
 */
@Component("marketActivity1009Handler")
public class MarketActivity1009Handler extends MarketActivityBaseSupportHandler{
	...
	public Map<String, Object> luckDraw(Map<String, Object> para) {
		logger.info("=====market activity 1009 luckDraw start=====");
		...
		if(lotteryTimes == null || (lotteryTimes.getTotalTimes().intValue() - lotteryTimes.getUsedTimes().intValue()) <= 0) {
			...
			para.put("message", "哎呦！您可进行的抽奖机会为0次，立即投资赢取抽奖机会吧！");
			return para;
		}
		...
		if((lotteryTimes.getTotalTimes().intValue() - lotteryTimes.getUsedTimes().intValue()) < 0) {
			logger.info("=====market activity 1009 luckDraw exception, because remaining times is negative number=====");
		...
		logger.info("=====market activity 1009 luckDraw end=====");
		return para;
	}
	...
	public Map<String,Object> investmentTender(Map<String,Object> para) {
		logger.info("=====market activity 1009 investmentTender, 投资获取大转盘抽奖次数 start=====");
		...
		int times = (int) (amount/MarketConstant.ACTIVITY_1009_REQUIRE_INVESTMENT_AMOUNT);
		logger.info("market activity 1009 the recordId:" + recordId + ",accountId:" + accountId + ",investment amount:" + amount);
		...
		if(times > 0){
			...
			if(mLotteryTimes != null){
				...
				mWheelLotteryTimes.setTotalTimes(times);
		...
		logger.info("=====market activity 1009 investmentTender, 投资送大转盘抽奖次数 start end=====");
		return para;
	}
}

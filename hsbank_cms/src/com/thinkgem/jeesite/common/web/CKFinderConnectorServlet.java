...
//		}
		...
		if ("Init".equals(command)){
			...
			if (startupPath!=null){
				...
				if (ss.length==2){
					String realPath = "";
					if(principalObject instanceof com.thinkgem.jeesite.modules.sys.security.SystemAuthorizingRealmFront.Principal) {
						realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL
								+ (com.thinkgem.jeesite.modules.sys.security.SystemAuthorizingRealmFront.Principal)principalObject + "/" + ss[0] + ss[1];
					}else if(principalObject instanceof Principal) {
						realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL
								+ (Principal)principalObject + "/" + ss[0] + ss[1];
					}
		...
		// 快捷上传，自动创建当前文件夹，并上传到该路径
			...
			if(principalObject instanceof Principal) {
				realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL
						+ (Principal)principalObject + "/" + type + (currentFolder != null ? currentFolder : "");
			}else if(principalObject instanceof com.thinkgem.jeesite.modules.sys.security.SystemAuthorizingRealmFront.Principal) {
				realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL
						+ (com.thinkgem.jeesite.modules.sys.security.SystemAuthorizingRealmFront.Principal)principalObject + "/" + type + (currentFolder != null ? currentFolder : "");
			}
			FileUtils.createDirectory(realPath);
		}
//		System.out.println("------------------------");
